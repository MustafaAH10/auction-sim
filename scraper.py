...
class Player(object):
    def __init__(self, player_id):
        ...
        self.url = "https://www.espncricinfo.com/player/ajinkya-rahane-{0}".format(
            str(player_id)
        )
    ...
    #     return [
    #         x.text
    #         for x in self.parsed_html.find(
    #             "div", class_="overview-teams-grid"
    #         ).find_all("h5")
    #     ]
    ...
    def get_career_averages(
        self, file_name=None, match_format=11, data_type="allround"
    ):
    ...
    def get_data(
        self, file_name=None, match_format=11, data_type="allround", view="match"
    ):
        """Get Player data match by match sorted by date
        ...
        if self.file_name is None:
            self.file_name = (
                f"{self.player_id}_{self.match_format}_{self.data_type}_{self.view}.csv"
            )
...
def main():
    player_id = "277916"  # Ajinkya Rahane's ID, replace with any player's ID
    try:
        ...
        print(f"Playing Role: {player.playing_role}")
        print(f"Batting Style: {player.batting_style}")
        print(f"Bowling Style: {player.bowling_style}")
        # print(f"Major Teams: {player.major_teams}")

        # Fetch and print specific data as needed
        # For example, to get career averages:
        player.get_career_averages()
        print(f"Career averages saved to: {player.file_name}")
